class GooglePlotManager {
  constructor(config) {
    ...
    if (!this.validateConfig()) {
      console.error("GooglePlotManager: Invalid configuration provided");
      return;
    }
  ...
  validateConfig() {
    ...
    const requiredProps = ["csrfToken"];
    const missing = requiredProps.filter((prop) => !this.config[prop]);
  ...
  getMapData() {
    ...
    if (this.formElements.plotSelect && this.formElements.plotSelect.value) {
            ...
            (results, status) => {
              if (status === "OK" && results.length) {
                ...
                address.location_name =
                  address.location_name || results[0].formatted_address;
                resolve(address);
              } else {
                console.warn(
  ...
  updateMapWithData(data) {
    ...
    // Adjust map bounds after addresses are processed
    setTimeout(() => {
      if (!bounds.isEmpty()) {
        this.map.fitBounds(bounds);
      } else {
        console.log("No valid coordinates found for map bounds");
      }
    }, 2000); // Increased timeout for better geocoding completion
  }
  ...
  /**
   * Set up enhanced Google Places autocomplete for address input.
  ...
  setupAutocomplete() {
    ...
    const autocomplete = new google.maps.places.Autocomplete(addressInput, {
      ...
      fields: ["formatted_address", "geometry", "name", "address_components"], // Specify required fields
    });
    ...
    autocomplete.addListener("place_changed", () => {
      ...
      if (!place.geometry || !place.geometry.location) {
        console.log(
          `No details available for: '${place.name || "selected place"}'`
        );
      ...
      if (
        ...
        // Optional: Auto-fill additional fields (if they exist)
        this.autofillAddressFields(place);
        ...
        console.warn("Invalid coordinates for selected place:", location);
        this.selectedPlace = null;
        this.clearTempMarker();
      }
    });
    ...
    addressInput.addEventListener("input", () => {
      // Clear temporary marker when user starts typing again
      if (this.tempMarker) {
        this.clearTempMarker();
        this.selectedPlace = null;
      }
    ...
    console.log("✅ Google Places Autocomplete initialized for address input");
  }

  /**
   * Auto-fill additional address fields if they exist in the form
   */
  autofillAddressFields(place) {
    ...
    Object.entries(fieldMappings).forEach(([componentType, fieldId]) => {
      const field = document.getElementById(fieldId);
      if (field && components[componentType]) {
        // Use short name for state if available
        const value =
          componentType === "administrative_area_level_1" &&
          components[componentType + "_short"]
            ? components[componentType + "_short"]
    ...
    const autoFilled = Object.keys(components).filter((key) =>
      document.getElementById(fieldMappings[key.replace("_short", "")])
    );

    if (autoFilled.length > 0) {
      console.log("✅ Auto-filled address fields:", autoFilled);
    }
  }
  ...
  showPlotOnMap(address) {
    ...
    if (
      ...
      alert("Cannot show this address on the map due to invalid coordinates.");
    }
  }
